...
export default function Header() {
  ...
  const currentLocale = pathParts.length > 0 && pathParts[0].length === 2 ? pathParts[0] : 'en';
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Brand Logo */}
            <Link href={currentLocale === 'en' ? '/' : `/${currentLocale}/`} className="flex items-center gap-2.5 group">
            ...
            <div className="flex md:hidden items-center gap-1.5">
              <button
                type="button"
                onClick={() => setLangModalOpen(true)}
                className="p-2 rounded-lg text-slate-700 hover:text-slate-900 hover:bg-slate-100 flex items-center gap-1 text-xs font-bold"
