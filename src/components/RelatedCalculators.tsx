...
export default function RelatedCalculators({
  ...
  const headingTitle = title || t('related.title', 'Explore Related Age & Date Calculators');
  const headingDesc = description || t('related.description', 'Quickly switch to specialized calculators for birthdays, milestone ages, date durations, and interval counting.');
  const openLabel = t('related.openCalculator', 'Open Calculator');

  const filtered = ALL_CALCULATORS.filter((calc) => {
    const slug = calc.href.replace(/^\//, '');
    return slug !== currentSlug && calc.href !== currentSlug;
  }).slice(0, limit);
